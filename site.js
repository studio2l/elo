...
var Category = /** @class */ (function () {
    ...
    // 그룹
    Category.prototype.GroupDir = function (prj, grp) {
        return this.groupRoot(prj) + "/" + grp;
    };
    Category.prototype.GroupsOf = function (prj) {
        var d = this.groupRoot(prj);
        return childDirs(d);
    };
    Category.prototype.CreateGroup = function (prj, grp) {
        var d = this.GroupDir(prj, grp);
        fs.mkdirSync(d);
    };
    // 유닛
    Category.prototype.UnitDir = function (prj, grp, unit) {
        return this.unitRoot(prj, grp) + "/" + unit;
    };
    Category.prototype.UnitsOf = function (prj, grp) {
        var d = this.unitRoot(prj, grp);
        return childDirs(d);
    };
    Category.prototype.CreateUnit = function (prj, grp, unit) {
        var d = this.UnitDir(prj, grp, unit);
        fs.mkdirSync(d);
        createDirs(d, this.unitSubdirs);
    };
    // 파트
    Category.prototype.PartDir = function (prj, grp, unit, part) {
        return this.partRoot(prj, grp, unit) + "/" + part;
    };
    Category.prototype.PartsOf = function (prj, grp, unit) {
        var d = this.partRoot(prj, grp, unit);
        return childDirs(d);
    };
    Category.prototype.CreatePart = function (prj, grp, unit, part) {
        var d = this.PartDir(prj, grp, unit, part);
        fs.mkdirSync(d);
        var subdirs = this.partSubdirs[part];
        if (!subdirs) {
            return;
        }
        createDirs(d, subdirs);
        // 해당 파트의 기본 태스크 생성
        var tasksInfo = this.defaultTasksInfo[part];
        if (tasksInfo) {
            for (var i in tasksInfo) {
                var ti = tasksInfo[i];
    ...
    // 태스크
    Category.prototype.TasksOf = function (prj, grp, unit, part) {
        var partdir = this.PartDir(prj, grp, unit, part);
        var progs = this.ProgramsOf(prj, grp, unit, part);
        if (!progs) {
            return {};
        }
        var tasks = {};
    ...
    Category.prototype.CreateTask = function (prj, grp, unit, part, task, ver, prog) {
        ...
        var progs = this.ProgramsOf(prj, grp, unit, part);
        var p = progs[prog];
        var scene = p.SceneName(prj, grp, unit, part, task, ver);
        var env = cloneEnv();
        var sceneEnv = this.SceneEnviron(prj, grp, unit, part, task);
        for (var e in sceneEnv) {
            env[e] = sceneEnv[e];
        }
        p.CreateScene(scene, env);
    };
    Category.prototype.OpenTask = function (prj, grp, unit, part, task, prog, ver, handleError) {
        var progs = this.ProgramsOf(prj, grp, unit, part);
        var p = progs[prog];
        if (!p) {
    ...
    // 씬 환경변수
    Category.prototype.SceneEnviron = function (prj, grp, unit, part, task) {
        var env = {
            "PRJ": prj,
            "GROUP": grp,
            "SHOT": unit,
    ...
    // 프로그램
    Category.prototype.ProgramsOf = function (prj, grp, unit, part) {
        // prj와 grp, unit은 아직 사용하지 않는다.
        var pgs = this.programs[part];
        if (!pgs) {
...
var Program = /** @class */ (function () {
    function Program(name, dir, ext, CreateScene, OpenScene) {
        this.Name = name;
        this.Dir = dir;
        this.Ext = ext;
    ...
    Program.prototype.SceneName = function (prj, grp, unit, part, task, ver) {
        var scene = this.Dir + "/" + prj + "_" + grp + "_" + unit + "_" + part + "_" + task + "_" + ver + this.Ext;
        return scene;
    };
    Program.prototype.ListTasks = function (prj, grp, unit, part) {
        var tasks = {};
        ...
        for (var _i = 0, files_1 = files; _i < files_1.length; _i++) {
            ...
            var prefix = prj + "_" + grp + "_" + unit + "_" + part + "_";
...
function newMayaAt(dir) {
    ...
    // CreateScene
    function (scene, env) {
        var cmd = siteRoot + "/runner/maya_create.sh";
    ...
    function (scene, env, handleError) {
        var cmd = siteRoot + "/runner/maya_open.sh";
        if (process.platform == "win32") {
...
function newHoudiniAt(dir) {
    ...
    // CreateScene
    function (scene, env) {
        var cmd = siteRoot + "/runner/houdini_create.sh";
        if (process.platform == "win32") {
    ...
    // OpenScene
    function (scene, env, handleError) {
        var cmd = siteRoot + "/runner/houdini_open.sh";
        if (process.platform == "win32") {
...
function newNukeAt(dir) {
    ...
    // CreateScene
    function (scene, env) {
        var cmd = siteRoot + "/runner/nuke_create.sh";
        if (process.platform == "win32") {
    ...
    // OpenScene
    function (scene, env, handleError) {
        var cmd = siteRoot + "/runner/nuke_open.sh";
        if (process.platform == "win32") {
...
function childDirs(d) {
    ...
    fs.readdirSync(d).forEach(function (f) {
        var isDir = fs.lstatSync(d + "/" + f).isDirectory();
        if (isDir) {
            cds.push(f);
        }
    });
...
function createDirs(parentd, subdirs) {
    ...
    for (var _i = 0, subdirs_1 = subdirs; _i < subdirs_1.length; _i++) {
        ...
        if (process.platform == "win32") {
            ...
            var specialBit = perm.substring(0, 1);
            var defaultBits = perm.substring(1, 4);
